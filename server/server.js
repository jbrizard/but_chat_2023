...
io.sockets.on('connection', function(socket)
{
	...
	{
		const userRoom = Array.from(Object.values(chess.rooms)).filter(room => room.players.includes(socket.name))
		console.log(userRoom[0].name);
	
		let data = chessGame.setupGame()
		io.to(userRoom[0].name).emit("startChessGame", data);
	})
