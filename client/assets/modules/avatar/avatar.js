...
$("#imagePreview").css(
  "background-image",
  "url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSTR3zZjipG0-Lf-MtJcieX_ASoCDA_6JfGxA&usqp=CAU)"
);
...
function upload(files) {
  socket.emit(
    "upload",
    { file: files[0], name: files[0] ? files[0].name : null },
    (status) => {
      ...
      $("body").append(
        `<p class="alertAvatar">${
          status.message === "success"
            ...
            : "Erreur lors de l'enregistrement de votre avatar"
        }</p>`
      );
      $(".alertAvatar").css(
        "background-color",
